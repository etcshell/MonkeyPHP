...
return array('timezone' => 'PRC', 'response_charset' => 'UTF-8', 'domain' => 'localhost', //网站主域名

    'cache' => array('default_provider' => 'apc', 'file' => '\Monkey\Cache\File', 'memcache' => '\Monkey\Cache\Memcache', 'apc' => '\Monkey\Cache\Apc',), 'cache_file' => array('expire' => 3600, //默认缓存时间
        'dir' => '/fileCache', //缓存文件的相对路径（相对临时缓存目录），留空为/filecache
        'filename' => 'data', 'filesize' => '15M', 'check' => false,), 'cache_memcache' => array('expire' => 3600, //默认缓存时间
        'host' => 'localhost', 'port' => 11211, 'persistent' => '', 'compressed' => FALSE,), 'cache_apc' => array('expire' => 3600, //默认缓存时间
    ),

    'database' => array('default_provider' => 'default', 'default' => '\Monkey\Database\Database',), 'database_default' => array('default_connection' => 'master', //链接池子
        'pool' => array( //本数组采用了键值倒置设计，目的是方便操作和提高检索速度
            //后面的值 1 是随便设置的值，只要是TRUE类型都可以
            'master' => 1,), //每个连接的键名可以为数字（省略或不省略都可以），比如 下面这个表示 links[0]
        'master' => array('protocol' => 'mysql', //数据库协议
            //dsn配置是可选的、优先的
            ...
            'dbname' => 'macaca', 'unix_socket' => '', 'charset' => 'utf8', //字符集设置
    ...
    'router' => array('default_provider' => 'default', 'default' => '\Monkey\Router\Router',), 'router_default' => array( //路由存贮配置，相对应用目录。
        ...
        'pattern_option' => array( //路由匹配时的编译标签，简记名（只能用一对花括号括起来）=>正则表达式（只能用一对括号括起来）
            '{i}' => "(\d+)", '{s}' => "([^\/]+)", '{year}' => "([1-2]\d{3})", '{month}' => "(1[0,1,2]|[1-9])", '{day}' => "([1-9]|[1,2][0-9]|3[0,1])", '{name}' => "(\w+)", '{zh|en}' => "(zh|en)", '{json}' => "(\.json)",), 'router_class_auto_prefix' => true, //自动将router表中类名加上前缀 \AppName\Controller\

        //三个选择：rewrite（需服务器支持）、pathinfo（需服务器支持）、get（传统方式）
        'search_mode' => 'rewrite', //get字段上的显式方法设置，如http://www.xxx.php?r=index
        'search_get' => 'r',),

    'session' => array('default_provider' => 'apc', 'file' => '\Monkey\Session\File', 'memcache' => '\Monkey\Session\Memcache', 'apc' => '\Monkey\Session\Apc',), 'session_file' => array( //Session键名前缀，保证每个应用的Session不会碰撞（尤其是使用Memcache作为存储介质时）。
    ...
    'shutdown' => array('default_provider' => 'default', 'default' => '\Monkey\Shutdown\Shutdown',), 'shutdown_default' => array(),

    'view' => array('default_provider' => 'default', 'default' => '\Monkey\View\View',), 'view_default' => array('charset' => 'UTF-8', //
        'template_root' => '/template', //相对于应用目录
        'compiled_root' => '/template_compiled',

        //分页栏配置：
        'page_style_name' => 'def', 'def_link' => '<a href="http://urlPre{number}">{text}</a>', 'def_link_ajax' => '<a href="javascript:ajaxActionName(\'http://urlPre{number}\')">{text}</a>', 'def_span_current' => '<span class="current_style">{number}</span>', 'def_span_total' => '<span class="total_style">共{number}页</span>', 'def_input_jump' => '转到<input type="text" class="jump_style" size="2" title="输入页码，按回车快速跳转" value="1" onkeydown="if(event.keyCode==13) {window.location=\'http://urlPre\'+this.value; doane(event);}" />', 'def_text_first' => '首页', //另外，图片可以设置为：'<img src="..." width="16" height="11" />'，下同
        'def_text_pre' => '上一页', 'def_text_next' => '下一页', 'def_text_last' => '尾页', 'def_layout' => 'pre-current-next', //'first-pre-current-next-last'， 'first-pre-list-next-last'（list包含current）
    ...
    'siteset' => array('site_enable' => true, //是否关闭网站 	否 是
