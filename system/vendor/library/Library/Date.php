...
class Date {
    ...
    /**
     * 增加天数
     * @param  int $dayNum 增加多少天
     * @return $this
     */
    public function addDay($dayNum = 1) {
        $dayNum = (int)$dayNum;
        $dayNum = $dayNum * 86400;
        $time = $this->getTime() + $dayNum;
    ...
    public function getDayOfMonthEnd() {
        $endDay = 31;
        if ($this->month == 2) {
            $endDay = $this->isLeapYear($this->year) ? 29 : 28;
        }
     ...
     * 计算2个日期的差值
     * 算法结果与sql查询结果相同
     * @param string $difDate 要比较的时间字符串
     * @param string $referDate 参考时间，默认为类中指定日期天的0时
     * @return int
     */
    public function getDifferenceOfSQL($difDate, $referDate = null) {
        $difDate = strtotime($difDate);
        $difDate = $difDate - $difDate % 86400;
        if (is_null($referDate)) {
            $referDate = mktime(0, 0, 0, $this->month, $this->day, $this->year);
        }
        else {
            $referDate = strtotime($referDate);
            $referDate = $referDate - $referDate % 86400;
        }
        return abs(($difDate - $referDate) / 86400);
    }
     ...
     * @return int
     */
    public function getDifference($difDate, $referDate = null) {
        $difDate = strtotime($difDate);
        if (is_null($referDate)) {
            $referDate = mktime(0, 0, 0, $this->month, $this->day, $this->year);
        }
        ...
        return ceil(abs(($difDate - $referDate) / 86400));
    }
