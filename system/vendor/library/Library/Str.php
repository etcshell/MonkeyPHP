...
final class Str {
     ...
     * @param string $suffixStr 尾缀字符串
     * @return string
     */
    public static function subText($str, $start, $length, $charset = null, $suffix = false, $suffixStr = '…') {
        if (empty ($str)) {
            return $str;
        }
        $a = array('&', '"', '<', '>', ' ');
        ...
        if (function_exists('mb_substr')) {
            if (mb_strlen($str, $charset) <= $start + $length) {
                ...
                $suffixLen = mb_strlen($suffixStr, $charset);
                self::fixLength($suffix, $length, $suffixLen);
            }
        ...
        elseif (function_exists('iconv_substr')) {
            if (iconv_strlen($str, $charset) <= $start + $length) {
                ...
                $suffixLen = iconv_strlen($suffixStr, $charset);
                self::fixLength($suffix, $length, $suffixLen);
            }
            ...
            if (isset($match[0][$length + $start])) {
                preg_match_all($re[$charset], $suffixStr, $suffixMatch);
                $suffixLen = count($suffixMatch[0]);
                self::fixLength($suffix, $length, $suffixLen);
            }
            else {
                $suffix = false;
        ...
        $suffix and $slice .= $suffixStr;
        return str_replace($a, $b, $slice);
    }

    private static function fixLength(&$suffix, &$length, $suffixLen) {
    ...
    public static function subHtml($body, $size) {
         ...
         */
        $htmlArray = array('left' => array(), 'right' => array());
        for ($i = 0; $i < $strlenVar; ++$i) {
            if (!$size) {
                break;
            }
            $currentVar = substr($body, $i, 1);
            if ($currentVar == '<') {
                // html 代码开始
                $htmlTag = 1;
                $htmlArrayStr = '';
                ...
                if ($currentVar == '>') {
                    ...
                    if (substr($htmlArrayStr, -1) != '/') {
                        ...
                        if ($f == '/') {
                            // 去掉 /
                            $htmlArray ['right'] [] = str_replace('/', '', $htmlArrayStr);
                        }
                        else if ($f != '?') {
                            ...
                            if (strpos($htmlArrayStr, ' ') !== false) {
                                ...
                                $htmlArray ['left'] [] = strtolower(current(explode(' ', $htmlArrayStr, 2)));
                            }
                            else {
                                /**
                                 ...
                                 * 统一转换为小写
                                 */
                                $htmlArray ['left'] [] = strtolower($htmlArrayStr);
                            }
                        }
                    }
                    // 字符串重置
            ...
            switch (true) {
                case (($ordVarc & 0xE0) == 0xC0) :
                    // 2 字节
                    $summaryString .= substr($body, $i, 2);
                    $i += 1;
                    break;
                case (($ordVarc & 0xF0) == 0xE0) :
                    // 3 字节
                    $summaryString .= substr($body, $i, 3);
                    $i += 2;
                    break;
                case (($ordVarc & 0xF8) == 0xF0) :
                    // 4 字节
                    $summaryString .= substr($body, $i, 4);
                    $i += 3;
                    break;
                case (($ordVarc & 0xFC) == 0xF8) :
                    // 5 字节
                    $summaryString .= substr($body, $i, 5);
                    $i += 4;
                    break;
                case (($ordVarc & 0xFE) == 0xFC) :
                    // 6 字节
                    $summaryString .= substr($body, $i, 6);
                    $i += 5;
                    break;
                default :
                    // 1 字节
                    $summaryString .= $currentVar;
            }
        }
        if ($htmlArray ['left']) {
            /**
             ...
             * 补充顺序应为：</p></b></h2>
             */
            $htmlArray ['left'] = array_reverse($htmlArray ['left']);
            foreach ($htmlArray ['left'] as $tag) {
                // 判断该标签是否出现在 right 中
                $key = array_search($tag, $htmlArray ['right']);
                if ($key !== false) {
                    // 出现，从 right 中删除该单元
                    unset ($htmlArray ['right'] [$key]);
                }
                else {
                    // 没有出现，需要补全
                    $summaryString .= '</' . $tag . '>';
     ...
     * @param bool $dataIsUtf8 TRUE（默认值）：原数据是UTF8编码的。FALSE：原数据是GB2312编码的。
     * @return string
     */
    public static function cn2tw($data, $dataIsUtf8 = true) {
        return self::cn($data, 1, $dataIsUtf8);
    }
     ...
     * @return string
     */
    public static function tw2cn($data, $dataIsUtf8 = true) {
        return self::cn($data, 0, $dataIsUtf8);
    }
     ...
     * @return string
     */
    private static function cn($data, $type = 0, $dataIsUtf8 = true) {
        $simcn =
        ...
        for ($i = 0, $length = strlen($str); $i < $length; $i++) {
            ...
            if ($thisValue < 128) {
                ...
                if (count($values) == $lookingFor) {
                    $unicode[] =
                        ($lookingFor == 3) ? ($values[0] % 16) * 4096 + ($values[1] % 64) * 64 + $values[2] % 64 :
     ...
     * @return array
     */
    public function splitText($str, $length, $charset = 'utf-8', $suffix = false, $suffixStr = '……') {
        $strLen = self::length($str, $charset);
        $n = ceil($strLen / $length);
        if ($n == 1) {
            return array($str);
        }
        $strArray = array();
        for ($i = 0; $i < $n; $i++) {
            $strArray[$i] = self::subText($str, $i * $length, $length, $charset, $suffix, $suffixStr);
        }
        return $strArray;
    }
     ...
     * @param int $pageMaxLength 结果数组中每项的最大显示字符数（忽略了Html代码）
     * @param string $charset utf-8|gb2312|gbk|big5 编码
     * @param array $tagArray 分割的字符或Html标记（注意字符的顺序），默认为array('</table>', '</div>', '</p>', '<br/>', '”。', '。', '.', '！', '……', '？',',')
     * @return array 数组
     */
    public function splitHtml($content, $pageMaxLength = 500, $tagArray = null, $charset = 'utf-8') {
        $a = array('&', '"', '<', '>', ' ');
        $b = array('&amp;', '&quot;', '&lt;', '&gt;', '&nbsp;');
        $str = str_replace($b, $a, $content);
        $strLen = Str::length($str, $charset);
        if ($strLen < $pageMaxLength) {
            return array($content);
        }
        $strLen = strlen($str);
        if (empty($tagArray)) {
            $tagArray = array('</table>', '</div>', '</p>', '<br/>', '”。', '。', '.', '！', '……', '？', ',');
        }
        $contentArray = array();
        $cutStart = $temp = 0;
        do {
            $i = 0;
            $cutStr = substr($str, $cutStart, $pageMaxLength);
            if (strlen($cutStr) < $pageMaxLength) {
                $contentArray[] = str_replace($a, $b, $cutStr);
                break;
            }
            foreach ($tagArray as $tag) {
                $i = strrpos($cutStr, $tag); //逆向查找第一个分页符的位置
                if ($i !== false) {
                    $contentArray[] = str_replace($a, $b, substr($cutStr, 0, $i) . $tag);
                    $cutStart += $i + strlen($tag);
                    break;
                }
            }
            if (!$i) {
                if ($cutStart + $pageMaxLength == $strLen - 1) {
                    $contentArray[] = str_replace($a, $b, $cutStr);
                    break;
                }
                else {
                    $temp = Str::subText(
                        $cutStr,
                        0,
                        Str::length($cutStr, $charset) - 1,
                        $charset,
                        false
                    );
                    $cutStart += strlen($temp);
                    $contentArray[] = str_replace($a, $b, $temp);
                }
            }
        } while (1);
        return $contentArray;
