...
class ExcelReader {
    ...
    // =========================================
    function dump($rowNumbers = false, $colLetters = false, $sheet = 0, $tableClass = 'excel') {
        $out = "<table class=\"$tableClass\" cellspacing=0>";
        if ($colLetters) {
            $out .= "<thead>\n\t<tr>";
            if ($rowNumbers) {
                $out .= "\n\t\t<th>&nbsp</th>";
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                if (!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
                    ...
                    if ($val == '') {
                        ...
                        $val =
                            htmlentities($val, ENT_NOQUOTES, $this->defaultEncoding); // 2012-04-19 maojianlw@139.com
    ...
    var $defaultFormat = SPREADSHEET_EXCEL_READER_DEF_NUM_FORMAT;
    var $columnsFormat = array();
    var $rowoffset = 1;
    var $coloffset = 1;
    ...
    function format_value($format, $num, $f) {
        // 49==TEXT format
        ...
        $color = "";
        $matches = array();
        $colorRegex = "/^\[(BLACK|BLUE|CYAN|GREEN|MAGENTA|RED|WHITE|YELLOW)\]/i";
        if (preg_match($colorRegex, $pattern, $matches)) {
            $color = strtolower($matches[1]);
            $pattern = preg_replace($colorRegex, "", $pattern);
        }
        ...
        $numberRegex = "/(\d+)(\.?)(\d*)/";
        if (preg_match($numberRegex, $pattern, $matches)) {
            $left = $matches[1];
            $dec = $matches[2];
            $right = $matches[3];
            if ($hasCommas) {
                ...
                $sprintfPattern = "%1." . strlen($right) . "f";
                $formatted = sprintf($sprintfPattern, $num);
            }
            $pattern = preg_replace($numberRegex, $formatted, $pattern);
        }
     ...
     */
    function __construct($file = '', $storeExtendedInfo = true, $outputEncoding = '') {
            ...
            switch ($code) {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_PALETTE:
                    ...
                    for ($coli = 0; $coli < $colors; $coli++) {
                        ...
                        $this->colors[0x07 + $coli] =
                            '#' . $this->myhex($colr) . $this->myhex($colg) . $this->myhex($colb);
                    }
                    break;
                        ...
                        if ($indexCode > 0) {
                            ...
                            if ($formatstr != "") {
                                ...
                                if (preg_match("/[^hmsday\/\-:\s\\\,AMP]/i", $tmp) == 0) { // found day and time format
                                    ...
                                    $formatstr = $tmp;
                                    $formatstr =
                                        str_replace(array('AM/PM', 'mmmm', 'mmm'), array('a', 'F', 'M'), $formatstr);
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                    $recOffset = $this->_GetInt4d($data, $pos + 4);
                    $recTypeFlag = ord($data[$pos + 8]);
                    $recVisibilityFlag = ord($data[$pos + 9]);
                    $recLength = ord($data[$pos + 10]);

                    if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
                        $chartype = ord($data[$pos + 11]);
                        if ($chartype == 0) {
                            $recName = substr($data, $pos + 12, $recLength);
                        }
                        else {
                            $recName = $this->encodeUTF16(substr($data, $pos + 12, $recLength * 2));
                        }
                    }
                    elseif ($version == SPREADSHEET_EXCEL_READER_BIFF7) {
                        $recName = substr($data, $pos + 11, $recLength);
                    }
                    $this->boundsheets[] = array('name' => $recName, 'offset' => $recOffset);
                    break;
    ...
    function getCellDetails($spos, $numValue, $column) {
        $xfindex = ord($this->data[$spos + 4]) | ord($this->data[$spos + 5]) << 8;
        ...
        if ($type == 'date') {
            ...
            $utcDays = floor(
                $numValue -
                ($this->nineteenFour ? SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 :
            ...
            $raw = $numValue;
            $fractionalDay =
                $numValue - floor($numValue) + .0000001; // The .0000001 is to fix for php/excel fractional diffs
            ...
            $string =
                date($format, mktime($hours, $mins, $secs, $dateinfo["mon"], $dateinfo["mday"], $dateinfo["year"]));
        }
        else if ($type == 'number') {
            $rectype = 'number';
