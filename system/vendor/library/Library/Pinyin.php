...
class Pinyin {
    ...
    private function _num2str($num) {
        ...
        elseif ($num < -20319 || $num > -10247) {
            ...
            return $this->_lib[abs($num)];
        }
    }

    /**
     * 汉字转化并输出拼音
     * @param string $zh_str 所要转化的汉字
     * @param boolean $utf8 汉字编码是否为utf8
     * @return mixed 成功时返回拼音，失败时为False
     */
    public function getPinyin($zh_str, $utf8 = true) {
        if (!$zh_str) {
            return false;
        }
        //编码转换.
        ...
        $pinyin = '';
        for ($i = 0; $i < $num; $i++) {
            $temp = ord(substr($zh_str, $i, 1));
            if ($temp > 160) {
                $temp2 = ord(substr($zh_str, ++$i, 1));
