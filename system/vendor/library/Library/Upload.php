...
class Upload {
    ...
    public function __construct($fileSystemCharset = null, $defaultDir = null, $filterType = null, $maxSize = -1, $overwrite = FALSE) {
        ...
        $this->defaultOptions = array('saveDir' => $defaultDir ? $defaultDir : dir_format(dirname(FRONT_FILE)) . '/upload', 'filterType' => (string)$filterType, 'maxSize' => ctype_digit($maxSize) ? $maxSize : -1, 'overwrite' => (bool)$overwrite);
    ...
    public function save($isAloneFile = true, array $options = array()) {
        ...
        if (empty($this->files) || empty($this->files[$o['field']])) {
            return $this->_notice(0, '文件没有上传成功!');
        }
    ...
    private function upload($i, &$f, &$o) {
        $this->_upload($i, is_array($f['name']) ? $f['name'][$i] : $f['name'], is_array($f['size']) ? $f['size'][$i] : $f['size'], is_array($f['tmp_name']) ? $f['tmp_name'][$i] : $f['tmp_name'], is_array($f['error']) ? $f['error'][$i] : $f['error'], is_array($o['saveDir']) ? $o['saveDir'][$i] : $o['saveDir'], is_array($o['saveName']) ? $o['saveName'][$i] : $o['saveName'], is_array($o['filterType']) ? $o['filterType'][$i] : $o['filterType'], is_array($o['maxSize']) ? $o['maxSize'][$i] : $o['maxSize'], is_array($o['overwrite']) ? $o['overwrite'][$i] : $o['overwrite']);
    }

    private function _upload($index, $upName, $upSize, $upTmp, $upError, $saveDir, $saveName, $filterType = null, $maxSize = 0, $overwrite = FALSE) {
    ...
    private function convertFileInformation($taintedFiles) {
        ...
        return $this->pathsToArray(preg_replace($pattern, '$1$3$2$4', $this->arrayToPaths($taintedFiles)));
    }
