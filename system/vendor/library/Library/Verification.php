...
class Verification {
    ...
    /**
     * 批量验证
     * @param array $rule //结构：array($rule,$str,$otherParams,……,$errorMsg)
     * @return mixed 成功时返回TRUE，失败时返回对应的$errorMsg或False，请用===比较
     * 可以批量加入$rules如：
     * $verification = new verification();
     * $verification->testing(
     *              array($rule1,$str1,$otherParams1,……,$errorMsg1),
     *              array($rule2,$str2,$otherParams2,……,$errorMsg2),
     *              ……
     *              );
     *
     */
    ...
    public function isUserName($username) {
        ...
        return preg_match("/^[_a-zA-Z0-9]+$/", $username)
            or preg_match("/^[\x{4e00}-\x{9fa5}_a-zA-Z0-9]+$/u", $username)
            or preg_match("/^[\x{2e80}-\x{9fff}_a-zA-Z0-9]+$/u", $username); //欧美语系、汉字、亚洲语系
    }
}