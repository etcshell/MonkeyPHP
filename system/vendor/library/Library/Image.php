...
class Image {
    ...
    public function verificationByCode($width = 120, $height = 40, $charset = '', $char_num = 4, $font_size = 25, $font_file = NULL) {
        ...
        for ($i = 0; $i < $char_num; $i++) {
            ...
            $chars .= $char;
            //随机生成颜色
            $font_color = imagecolorallocate($image, rand(0, 120), rand(0, 120), rand(0, 220));
            //把生成的字符写入到图形中
            imagettftext($image, $font_size, rand(-5, 15), $font_size * $i + 10, $height - 10, $font_color, $font_file, $char);
        }
        //添加像素点干扰
        for ($i = 0; $i < 80; $i++) {
            $point_color = imagecolorallocate($image, rand(0, 230), rand(0, 230), rand(0, 240));
            imagesetpixel($image, rand(0, $width), rand(0, $height), $point_color);
        }
        //添加线干扰
        for ($i = 0; $i < 8; $i++) {
            $line_color = imagecolorallocate($image, rand(0, 180), rand(0, 180), rand(0, 200));
    ...
    public function thumb($image, $thumb_filePath = null, $type = '', $thumb_width_max = 200, $thumb_height_max = 50, $interlace = true) {
        ...
        if (function_exists('ImageCopyResampled')) {
            ...
            imagecopyresized($thumb_image, $source_image, 0, 0, 0, 0, $thumb_width, $thumb_height, $source_width, $source_height);
        }
    ...
    public function waterByPicture($image_path, $water_path, $water_position_option = 9, $water_trans_color = array(), $alpha = 85, $result_path = NULL) {
        ...
        //设置透明色
        if (!empty($water_trans_color) && in_array($waterInfo['type'], array('gif', 'png', 'jpeg'))) {
            $trans_color = imagecolorallocate($water_handler, $water_trans_color[0], $water_trans_color[1], $water_trans_color[2]);
            imagecolortransparent($water_handler, $trans_color);
        }
        $this->_water($image_handler, $water_handler, $result_path, $imageInfo['type'], $imageInfo['width'], $imageInfo['height'], $waterInfo['width'], $waterInfo['height'], $water_position_option, $alpha, ($waterInfo['type'] == 'png'));
        return TRUE;
    }
    ...
    public function waterByText($image_path, $water_text, $result_path = NULL, $water_position_option = 9, $alpha = 85, $font_file = NULL, $font_style = array('size' => 12, 'color' => array(192, 192, 192), 'angle' => 0)) {
        ...
        $image_handler = $imageFun($image_path);
        //设置水印文本
        $water_text_box = imagettfbbox($font_style['size'], $font_style['angle'], $font_file, $water_text);
        //取得文本框大小
        ...
        $background = imagecolorallocate($water_handler, 255 - $font_style['color'][0], 255 - $font_style['color'][1], 255 - $font_style['color'][2]);
        //设置背景颜色
        imagefill($water_handler, 0, 0, $background); //填充背景颜色
        $text_color = imagecolorallocate($water_handler, $font_style['color'][0], $font_style['color'][1], $font_style['color'][2]);
        //设置文本颜色
        imagettftext($water_handler, $font_style['size'], $font_style['angle'], abs($water_text_box[0]) + 3, abs($water_text_box[7]) + 3, $text_color, $font_file, $water_text);
        //写入文本
        $this->_water($image_handler, $water_handler, $result_path, $imageInfo['type'], $imageInfo['width'], $imageInfo['height'], $water_text_w, $water_text_h, $water_position_option, $alpha);
        return TRUE;
    }
    ...
    public function waterByTextOfEmboss($image_path, $water_text, $font_file, $font_size = 50, $font_angle = 0, $emboss = TRUE, $water_position_option = 9, $alpha = 60, $result_path = NULL) {
        ...
        $image_handler = $imageFun($image_path);
        //设置水印文本
        $water_text_box = imagettfbbox($font_size, $font_angle, $font_file, $water_text);
        //取得文本框大小
        $water_text_w = abs($water_text_box[2] - $water_text_box[6]) + 10; //为margin_right加5px,
        $water_text_h = abs($water_text_box[3] - $water_text_box[7]) + 10;
        $water_handler = imagecreatetruecolor($water_text_w, $water_text_h); //创建文本框
        ...
        if ($emboss) {
            for ($i = 0; $i < 5; $i++) {
                $color = 65 + $i * 40;
                imagettftext($water_handler, $font_size, $font_angle, $pos[0] + 6 - $i, $pos[1] + 6 - $i, imagecolorallocate($water_handler, $color, $color, $color), $font_file, $water_text);
                //写入文本
            }
            $pos[0] += 1;
            $pos[1] += 1;
            ...
            for ($i = 0; $i < 5; $i++) {
                ...
                imagettftext($water_handler, $font_size, $font_angle, $pos[0] + $i, $pos[1] + $i, imagecolorallocate($water_handler, $color, $color, $color), $font_file, $water_text);
                //写入文本
            }
            $pos[0] += 6;
            $pos[1] += 6;
        }
        imagettftext($water_handler, $font_size, $font_angle, $pos[0], $pos[1], imagecolorallocate($water_handler, $color, $color, $color), $font_file, $water_text);
        //写入文本
