...
class Spyc {
    ...
    public function dump($array, $indent = false, $wordwrap = false) {
        ...
        foreach ($array as $key => $value) {
            $string .= $this->yamlize($key, $value, 0);
        }
        return $string;
    }
     ...
     */
    private function yamlize($key, $value, $indent, $previousKey = -1) {
        if (is_array($value)) {
            // It has children.  What to do?
            // Make it the right kind of item
            $string = $this->dumpNode($key, NULL, $indent, $previousKey);
            // Add the indent
        ...
        elseif (!is_array($value)) {
            ...
            $string = $this->dumpNode($key, $value, $indent, $previousKey);
        }
        return $string;
    }
    ...
    private function yamlizeArray($array, $indent) {
        if (is_array($array)) {
            ...
            $previousKey = -1;
            foreach ($array as $key => $value) {
                $string .= $this->yamlize($key, $value, $indent, $previousKey);
                $previousKey = $key;
            }
            return $string;
        }
     ...
     * @param $indent The indent of the current node
     */
    private function dumpNode($key, $value, $indent, $previousKey = -1) {
        ...
        if (is_int($key) && $key - 1 == $previousKey) {
            // It's a sequence
            $string = $spaces . '- ' . $value . "\n";
        }
        else {
    ...
    private function doFolding($value, $indent) {
        ...
        if ($this->dumpWordWrap === 0) {
            return $value;
        }

        if (strlen($value) > $this->dumpWordWrap) {
            $indent += $this->dumpIndent;
    ...
    private function toType($value) {
        ...
        elseif (preg_match('/^\\[(.+)\\]$/', $value, $matches)) {
            ...
            $explode = $this->inlineEscape($matches[1]);
        ...
        if (!empty($savedStrings)) {
            ...
            foreach ($explode as $key => $value) {
                while (strpos($value, 'YAMLString') !== false) {
                    $explode[$key] = preg_replace('/YAMLString/', $savedStrings[$i], $value, 1);
                    ++$i;
                    $value = $explode[$key];
                }
            }
    ...
    private function literalBlockContinues($line, $lineIndent) {
        ...
        if ($this->getIndent($line) > $lineIndent) {
            return true;
        }
        return false;
    }
    ...
    private function addArray($array, $indent) {
        ...
        // Unfolding inner array tree as defined in $this->arrpath.
        //$arr = $this->result; $tree[0] = $arr; $i = 1;

        $tempPath = Spyc::flatten($this->path);
        eval ('$arr = $this->result' . $tempPath . ';');


        if ($this->containsGroupAlias) {
            do {
                if (!isset($this->SavedGroups[$this->containsGroupAlias])) {
                    echo "Bad group name: $this->containsGroupAlias.";
                    break;
                }
                $groupPath = $this->SavedGroups[$this->containsGroupAlias];
