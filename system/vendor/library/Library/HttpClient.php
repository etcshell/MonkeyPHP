...
class HttpClient {
    ...
    public function __construct($defaultPort = 80, $defaultFlen = 8192, $block = true) {
        ...
        $this->defaultFlen = (int)$defaultFlen;
    ...
    public function get($url, $ip = '', $timeout = 15, $cookie = '', $freadLen = 0) {
        if ($freadLen > $this->defaultFlen) {
            $freadLen = $this->defaultFlen;
        }
        $this->http($url, '', $ip, $timeout, $cookie, $freadLen);
    }
    ...
    public function post($post, $url, $ip = '', $timeout = 15, $cookie = '', $freadLen = 0) {
        ...
        $this->http($url, $post, $ip, $timeout, $cookie, $freadLen);
    }
     ...
     * @return string
     */
    private function http($url, $post, $ip = '', $timeout = 15, $cookie = '', $freadLen = 0) {
        //处理参数
        ...
        $httpHeader = ($post !== '')
                ? $this->postHeader($url, $post, $ip, $port, $cookie)
                : $this->getHeader($url, $post, $ip, $port, $cookie);
        //socket
        ...
        //fread
        $freadLen = ($freadLen == 0) ? $this->defaultFlen : $freadLen;
        $isHttpHeader = false;
        $stopFread = false;
        while (!feof($httpFp) && !$stopFread) {
            if ((!$isHttpHeader) &&
                ($tempHttpReturn = @fgets($httpFp)) &&
                ($tempHttpReturn == "\r\n" || $tempHttpReturn == "\n")
            ) {
                $isHttpHeader = true;
            }
     ...
     * @return string
     */
    private function postHeader($url, $post, $ip, $port = 80, $cookie = '') {
        $httpHeader = '';
     ...
     * @return string
     */
    private function getHeader($url, $post, $ip, $port = 80, $cookie = '') {
        $httpHeader = '';
