...
class UBB {
    ...
    public function ubb2html($sUBB) {
        ...
        $sHtml = preg_replace_callback(
            '/\[code\s*(?:=\s*((?:(?!")[\s\S])+?)(?:"[\s\S]*?)?)?\]([\s\S]*?)\[\/code\]/i',
            array(__CLASS__, 'saveCodeArea'),
            $sHtml
        );
        ...
        $sHtml = preg_replace_callback(
            '/\[size\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/i',
            array(__CLASS__, 'getSizeName'),
            $sHtml
        );
        $sHtml = preg_replace('/\[font\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/i', '<span style="font-family:$1;">', $sHtml);
        $sHtml =
            preg_replace('/\[back\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/i', '<span style="background-color:$1;">', $sHtml);
        $sHtml = preg_replace('/\[\/(color|size|font|back)\]/i', '</span>', $sHtml);
        for ($i = 0; $i < 3; $i++) {
            $sHtml = preg_replace(
                '/\[align\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\](((?!\[align(?:\s+[^\]]+)?\])[\s\S])*?)\[\/align\]/',
                '<p align="$1">$2</p>',
                $sHtml
            );
        }
        $sHtml =
            preg_replace('/\[img\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/img\]/i', '<img src="$1" alt="" />', $sHtml);
        $sHtml = preg_replace_callback(
            '/\[img\s*=([^,\]]*)(?:\s*,\s*(\d*%?)\s*,\s*(\d*%?)\s*)?(?:,?\s*(\w+))?\s*\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*)?\s*\[\/img\]/i',
            array(__CLASS__, 'getImg'),
            $sHtml
        );
        $sHtml = preg_replace_callback(
            '/\[emot\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\/\]/i',
            array(__CLASS__, 'getEmot'),
            $sHtml
        );
        $sHtml =
            preg_replace('/\[url\]\s*(((?!")[\s\S])*?)(?:"[\s\S]*?)?\s*\[\/url\]/i', '<a href="$1">$1</a>', $sHtml);
        $sHtml = preg_replace(
            '/\[url\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]\s*([\s\S]*?)\s*\[\/url\]/i',
            '<a href="$1">$2</a>',
            $sHtml
        );
        $sHtml = preg_replace(
            '/\[email\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/email\]/i',
            '<a href="mailto:$1">$1</a>',
            $sHtml
        );
        $sHtml = preg_replace(
            '/\[email\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]\s*([\s\S]+?)\s*\[\/email\]/i',
            '<a href="mailto:$1">$2</a>',
            $sHtml
        );
        ...
        $sHtml = preg_replace('/\[\/quote\]/i', '</blockquote>', $sHtml);
        $sHtml = preg_replace_callback(
            '/\[flash\s*(?:=\s*(\d+)\s*,\s*(\d+)\s*)?\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/flash\]/i',
            array(__CLASS__, 'getFlash'),
            $sHtml
        );
        $sHtml = preg_replace_callback(
            '/\[media\s*(?:=\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*(\d+)\s*)?)?\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/media\]/i',
            array(__CLASS__, 'getMedia'),
            $sHtml
        );
        $sHtml = preg_replace_callback(
            '/\[table\s*(?:=(\d{1,4}%?)\s*(?:,\s*([^\]"]+)(?:"[^\]]*?)?)?)?\s*\]/i',
            array(__CLASS__, 'getTable'),
            $sHtml
        );
        $sHtml =
            preg_replace_callback('/\[tr\s*(?:=(\s*[^\]"]+))?(?:"[^\]]*?)?\s*\]/i', array(__CLASS__, 'getTR'), $sHtml);
        $sHtml = preg_replace_callback(
            "/\[td\s*(?:=\s*(\d{1,2})\s*,\s*(\d{1,2})\s*(?:,\s*(\d{1,4}%?))?)?\s*\]/i",
            array(__CLASS__, 'getTD'),
            $sHtml
        );
        $sHtml = preg_replace("/\[\/(table|tr|td)\]/i", '</$1>', $sHtml);
        $sHtml =
            preg_replace("/\[\*\]((?:(?!\[\*\]|\[\/list\]|\[list\s*(?:=[^\]]+)?\])[\s\S])+)/i", '<li>$1</li>', $sHtml);
        $sHtml = preg_replace_callback(
            '/\[list\s*(?:=\s*([^\]"]+))?(?:"[^\]]*?)?\s*\]/i',
            array(__CLASS__, 'getUL'),
            $sHtml
        );
