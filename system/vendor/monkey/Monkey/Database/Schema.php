...
class Schema {
    ...
    public function renameField($tableName, $fieldName, $newFieldName) {
        if (!$this->existsField($tableName, $fieldName) || (($fieldName != $newFieldName) && $this->existsField($tableName, $newFieldName))
        ) {
            return false;
        }
    ...
    public function addPrimaryKey($tableName, $fields) {
        ...
        return $this->connection->query('ALTER TABLE {:' . $tableName . ':} ADD PRIMARY KEY (' . $this->createKeySql($fields) . ')')->isSuccess();
    }
    ...
    protected function explainSchema($databaseName) {
        ...
        foreach ($info as $v) {
            $this->schema[$v->table_name][$v->column_name] = array('description' => $v->column_comment, 'type' => $v->data_type, 'default' => $v->column_default, 'not null' => $v->is_nullable == 'NO',);
        }
        return $this;
    }
}
