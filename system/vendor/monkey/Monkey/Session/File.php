...
class File extends SessionAbstract {
    ...
    public function __construct($app) {
        ...
        $this->TIME = $app->TIME;
        $this->cachePath = dir_format($app->DIR . ($config['dir'] ? $config['dir'] : '/sessionCache'));
    ...
    public function read($sessionId) {
        $out = '';
        if ($this->fetch($this->storageKey($sessionId), $out)) {
            return $out;
        }
        else {
            return '';
    ...
    public function write($sessionId, $data) {
        return $this->store($this->storageKey($sessionId), $data, $this->expire);
    }
    ...
    public function destroy($sessionId) {
        return $this->delete($this->storageKey($sessionId));
    }
    ...
    public function fetch($key, &$result) {
        $result = null;
        if (!$this->actionFetch(md5($key), $content)) {
            return false;
        }
    ...
    public function store($key, $value, $time = -1) {
        ...
        return $this->actionStore(md5($key), $data); //存储数据
    }
    ...
    public function delete($key) {
        return $this->actionDelete(md5($key));
    }
    ...
    private function allSchemas() {
        ...
        for ($i = 0; $i < 16; $i++) {
            ...
            $info = unpack(
                'V1' . implode('/V1', $this->schemaStruct),
                fread($this->rs, $this->schemaItemSize)
            );
            if (!$info['size']) {
                return $schema;
            }
    ...
    private function create() {
        try {
            ...
            $this->error(__METHOD__ . ':创建缓存文件失败[ ' . $this->cacheFile . ' ]');
        }
        fseek($this->rs, 0);
    ...
    private function createNode($pos, $data) {
        $this->puts(
            $pos * $this->idxNodeSize + $this->idxNodeBase,
            pack(
                'V1V1V1V1V1V1H*',
                $data['next'],
                $data['prev'],
     ...
     * @return bool
     */
    private function actionDelete($md5Key, $pos = false) {
     ...
     * @return bool
     */
    private function actionFetch($md5Key, &$return) {
        ...
        $this->setSchema(
            $schemaId,
            'hits',
            $this->getSchema($schemaId, 'hits') + 1
        );
    ...
    private function freeNode($pos) {
        ...
        $this->puts(
            $pos * $this->idxNodeSize + $this->idxNodeBase,
            pack('V', $prevFreeNode) . str_repeat("\0", $this->idxNodeSize - 4)
        );
    ...
    private function getNode($offset) {
        ...
        $info = unpack(
            'V1next/V1prev/V1data/V1size/V1lru_right/V1lru_left/H*key',
    ...
    private function lruDelete($info) {
        if ($info['lru_right']) {
            ...
            $this->setSchema(
                $this->getSizeSchemaId($info['size']),
                'lru_tail',
                $info['lru_left']
            );
        }
    ...
    private function lruPop($schemaId) {
        ...
        $this->actionDelete($info['key'], $info['offset']);
    ...
    private function setNode($pos, $md5Key, $value) {
        ...
        if (isset($this->nodeStruct[$md5Key])) {
            return $this->puts(
                $pos * $this->idxNodeSize + $this->idxNodeBase + $this->nodeStruct[$md5Key][0],
                pack($this->nodeStruct[$md5Key][1], $value)
            );
        }
        else {
            return false;
    ...
    private function setNodeRoot($md5Key, $value) {
        return $this->puts(
            hexdec(substr($md5Key, 0, 4)) * 4 + $this->idxBasePos,
            pack('V', $value)
        );
    }
    ...
    private function setSchema($schemaId, $md5Key, $value) {
        $info = array_flip($this->schemaStruct);
        return $this->puts(
            60 + $schemaId * $this->schemaItemSize + $info[$md5Key] * 4,
            pack('V', $value)
        );
    }
     ...
     * @return bool
     */
    private function actionStore($md5Key, $data) {
    ...
    private function workat($file) {
        ...
        if (!file_exists($this->cacheFile)) {
            ...
            $this->rs = fopen($this->cacheFile, 'rb+') or
            $this->error(__METHOD__ . ':不能打开缓存文件[ ' . realpath($this->cacheFile) . ' ]');
