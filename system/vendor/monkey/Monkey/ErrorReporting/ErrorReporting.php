...
class ErrorReporting {
    ...
    public function __construct($app) {
        ...
        $dir =
            (isset($config['errorTemplate']) and !empty($config['errorTemplate'])) ?
                $app->DIR . $config['errorTemplate'] : strtr(__DIR__, DIRECTORY_SEPARATOR, '/') . '/errorTemplate';
    ...
    public function show403() {
        $errorInfo['code'] = '403';
        $errorInfo['msg'] = '你无权访问！';
        $this->show($errorInfo, $this->config['error_tpl_403']);
    }
    ...
    public function show404() {
        ...
        $this->show($errorInfo, $this->config['error_tpl_404']);
    }
    ...
    public function showError(array $errorInfo, $exit = false) {
        ...
        if ($this->app->DEBUG) { //显示所有错误信息
            $this->show($errorInfo, $this->config['error_tpl_debug']);
            ...
            $this->show($outErrorInfo, $this->config['error_tpl_public']);
        }
