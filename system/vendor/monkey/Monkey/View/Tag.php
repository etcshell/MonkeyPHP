...
class Tag {
    ...
    public static function getCallbackTag() {
        return array(
            ...
            /*{myapi::method pname1=pvalue1 pname2=pvalue2 ...}替换成 <?php echo call_user_func(array('appName\\LabelApi\\myapi', 'method'),array("pname1"=>pvalue1,"pname2"=>pvalue2, ... )); ?>*/
            /*<?php echo Label\$1::$2( template::regParam("$3") ); ?>*/
            array(
                '/\{([a-z_][a-z0-9_]*)::([a-z_][a-z0-9_]*)((\s+[a-z_][a-z0-9_]*=\S+)*)\s*\}/i',
                array(__CLASS__, 'apiString')
            ),
            //循环输出型API调用标签，返回数组，用于循环输出 需要结束标签
            /*<!--myapi::method pname1=pvalue1 pname2=pvalue2 ...-->替换成 <?php $myapiMethod=call_user_func(array('appName\\LabelApi\\myapi', 'method'),array("pname1"=>pvalue1,"pname2"=>pvalue2, ... )); if(is_array($myapiMethod)) foreach($myapiMethod as $apikey => $apival){ ?>*/
            array(
                '/\<!--([a-z_][a-z0-9_]*)::([a-z_][a-z0-9_]*)((\s+[a-z_][a-z0-9_]*=\S+)*)\s*-->/i',
                array(__CLASS__, 'apiArray')
            ),
            //数组标签解析
            /*{$arr.key}替换成<?php echo $array["key"]...;?> */
            array('/\{(\$[a-z_][a-z0-9_]*)((\.[\$a-z_][a-z0-9_]*)+)*\}/i', array(__CLASS__, 'arrayKeyEcho')),
            array(
                '/(<\?\s*php\s.*)(\$[a-z_][a-z0-9_]*)((\.[\$a-z_][a-z0-9_]*)+)(.*\?>)/i',
                array(__CLASS__, 'arrayKey')
            ),
        );
    }
    ...
    public static function apiString($matches) {
        return
            '<?php echo call_user_func(array(\'' . self::$appName . '\\\\LabelApi\\\\' . $matches[1] . '\', \'' .
            $matches[2] .
            '\'),' .
            self::regParam($matches[3]) .
            ' ); ?>';
    }
    ...
    public static function apiArray($matches) {
        ...
        return
            '<?php ' . $arr . '=call_user_func(array(\'' . self::$appName . '\\\\LabelApi\\\\' .
            $matches[1] .
            '\', \'' .
            $matches[2] .
            '\'),' .
