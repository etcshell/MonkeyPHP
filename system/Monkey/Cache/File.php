 ...
 * @version    $Id: \Monkey\Cache\File.php 版本号 2013-03-30 $
 */
final class File implements _Interface
{
    ...
    {
        $this->_cache_path=$config['dir']?$config['dir']:APP.'/temp/fileCache';
        ...
        if(!dir_check($this->_cache_path)) self::_error('缓存目录校验失败。');
    ...
    public function fetch($key, &$result){
        ...
        if($time != -1 && TIME >= $time) return false;
    ...
    private function _create(){
        try{
            ...
            self::_error(__METHOD__.':创建缓存文件失败[ '.$this->_cache_file.' ]');
        }
        fseek($this->_rs,0);
    ...
    private function _dalloc($schema_id,$lru_freed=false){
        ...
        if($free){ //如果lru里有链表
            ...
            self::_error(__METHOD__.':弹出lru区（最少使用区），空间被释放了');
        }else{
            ...
            if($size+$ds_offset > $this->_max_size){
                ...
                self::_error(__METHOD__.':不能分配存储空间');
            }else{
    ...
    private function _format($truncate=false){
        !$this->_lock_cache_file(true,true) && self::_error(__METHOD__.':不能锁定缓存文件');
        if($truncate){
            $this->_seek(0);
    ...
    private function _free_dspace($size,$pos){
        ($pos>$this->_max_size) && self::_error (__METHOD__.':释放缓存空间时溢出['.$pos.']');
    ...
    private function _lru_pop($schema_id){
        ...
        if(!$this->_get_schema($schema_id,'free')){
            self::_error(__METHOD__.':弹出lru区（最少使用区），但是空间没有被释放');
        }
        return $info;
    }
    ...
    private function _parse_str_size($str_size,$default){
        ...
        case 'g':
            if($match[1]>1) self::_error(__METHOD__.':设置缓存大小时越界，最大只能支持【1G】');
            $size = $match[1]<<30;
            break;
        case 'm':
            $size = $match[1]<<20;
        ...
        if($size<=0) self::_error(__METHOD__.':设置缓存大小时越界，缓存文件大小为0则无意义！');
    ...
    private function _puts($offset,$data){
        if($offset >= $this->_max_size*1.5){
            self::_error(__METHOD__.':向缓存文件中写入数据时，缓存数据的偏移量$offset【'.$offset.'】超出配额！');
        }
        $this->_seek($offset);
    ...
    private function _store($md5_key,$data){
        if(!$this->_lock_cache_file(true))self::_error(__METHOD__.':不能锁定文件!');
        $size = strlen($data);
        //get list_idx
        ...
        if($dataoffset>$this->_max_size) self::_error(__METHOD__.':分配缓存空间时出错！');
    ...
    private function _workat($file){
        ...
        if(!file_exists($this->_cache_file)){
            ...
            $this->_rs = fopen($this->_cache_file,'rb+') or self::_error(__METHOD__.':不能打开缓存文件[ '.realpath($this->_cache_file).' ]');
