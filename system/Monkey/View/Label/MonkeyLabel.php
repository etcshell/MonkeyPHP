...
/**
 * Framework默认标签方案\Framework\Label\FrameworkLabel
 * @package    Framework\Label
 * @author     HuangYi
 * @copyright  Copyright (c) 2011-07-01——2013-03-30
 * @license    New BSD License
 * @version    $Id:\Framework\Label\FrameworkLabel.php 版本号 2013-03-30 $
 */
class FrameworkLabel implements _Interface
{
    /**
     * 直接替换型标签
     * @return array
    ...
    public static function replaceTag(){
        return array(
            //添加常量解析标签：
            array('/__[A-Z_]+__/', '<?php if(defined("$0")) echo $0; else echo "$0"; ?>'),
            /*添加变量解析标签：将变量{$name}替换成<?php  echo $name;?> */
            ...
            /*<!--elseif/$name==2-->替换成 <?php } elseif ($name==2){ ?>*/
            array('/<!--elseif\/\s*(.+?)-->/i', '<?php } elseif ($1) { ?>' ),
            /*<!--else/-->替换成 <?php } else { ?>*/
    ...
    public static function callbackTag(){
        return array(
            //直接输出型API调用标签，返回字符串或数字等标量 不需要结束标签
            /*{myapi::method pname1=pvalue1 pname2=pvalue2 ...}替换成 <?php echo call_user_func(array('Label\\myapi', 'method'),array("pname1"=>pvalue1,"pname2"=>pvalue2, ... )); ?>*//*<?php echo Label\$1::$2( template::reg_param("$3") ); ?>*/
            array('/\{([a-z_][a-z0-9_]*)::([a-z_][a-z0-9_]*)((\s+[a-z_][a-z0-9_]*=\S+)*)\s*\}/i', array(__CLASS__,'apiString')),

            //循环输出型API调用标签，返回数组，用于循环输出 需要结束标签
            /*<!--myapi::method pname1=pvalue1 pname2=pvalue2 ...-->替换成 <?php $myapi_method=call_user_func(array('Label\\myapi', 'method'),array("pname1"=>pvalue1,"pname2"=>pvalue2, ... )); if(is_array($myapi_method)) foreach($myapi_method as $apikey => $apival){ ?>*/
    ...
    public static function apiString($matches){
        return '<?php echo call_user_func(array(\'Label\\\\'.$matches[1].'\', \''.$matches[2].'\'),'.self::regParam($matches[3]).' ); ?>';
    }
    public static function apiArray($matches){
        $arr='$'.$matches[1].'_'.$matches[2];
        return '<?php '.$arr.'=call_user_func(array(\'Label\\\\'.$matches[1].'\', \''.$matches[2].'\'),'.self::regParam($matches[3]).' ); if(is_array('.$arr.')) foreach('.$arr.' as $apikey => $apival){?>';
    }
    public static function arrayKeyFix($matches){
        $var_name=$matches[1];$keys=$matches[2];
    ...
    private static function getOneKey(&$keys){
        ...
        if($key[0]=='"' || $key[0]=='\'' || $key[0]=='$' ) return '['.$key.']';
        elseif(is_numeric($key)) return '['.$key.']';
        else return '["'.$key.'"]';
    }
    private static function regParam($param_str=''){
        $param_str=trim($param_str);
        if(empty($param_str))return 'null';
        $params='';
    ...
    private static function getOneParam(&$str){
        ...
        if($str[0]=='"' or $str[0]=='\''){
            ...
            if(!$offset)return false;
            if($str[$offset-1]=='\\')$offset=strpos($str,$str[0],$offset+1);
            if(!$offset)return false;
