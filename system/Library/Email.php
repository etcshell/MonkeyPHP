 ...
 * @version    $Id: email.class.php 版本号 2013-1-1  $
 *
 */
final class Email {
    var $smtp_port;
     ...
     */
    public function sendEmail($to, $from, $subject = "", $body = "", $type="HTML"
            , $cc = "", $bcc = "", $additional_headers = "") {
        ...
        if ($this->smtpSend($this->host_name
                , $mail_from, $rcpt_to, $header, $body)) {
    ...
    {
        $this->logWrite("Trying to ".$this->relay_host
                .":".$this->smtp_port."\n");
        $this->sock
                = @fsockopen($this->relay_host, $this->smtp_port
                        , $errno, $errstr, $this->time_out);
        if (!($this->sock && $this->smtpOK())) {
        $this->logWrite("Error: Cannot connenct to relay host "
                .$this->relay_host."\n");
    ...
    private function smtpSockopenMX($address) {
        ...
        $this->sock = @fsockopen($host, $this->smtp_port
                , $errno, $errstr, $this->time_out);
        if (!($this->sock && $this->smtpOK())) {
        ...
        $this->logWrite("Error: Cannot connect to any mx hosts ("
                .implode(", ", $MXHOSTS).")\n");
        return FALSE;
    }
    ...
    private function logWrite($message) {
        ...
        $message = date("M d H:i:s ")
                .get_current_user()."[".getmypid()."]: ".$message;
        if (!@file_exists($this->log_file)
                || !($fp = @fopen($this->log_file, "a"))) {
        $this->smtpDebug("Warning: Cannot open log file \""
                .$this->log_file."\"\n");
        return FALSE;
        }
        flock($fp, LOCK_EX);
    ...
    private function getAttachType($image_tag) { //
        ...
        $extension=substr($image_tag
                ,strrpos($image_tag,".")
                ,strlen($image_tag)-strrpos($image_tag,"."));
        switch($extension){
        case ".gif":
