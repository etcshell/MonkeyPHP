 ...
 * @version    $Id: str.class.php 版本号 2013-1-1 $
 *
 */
final class Str
{
     ...
     */
    public static function hasSubstr($str, $fullstr) {
        $fullstr = (string) $fullstr;
        $str = (string) $str;
        return strstr($fullstr,$str)===false ? false : true;
    }
    /**
     * @static
     * 过滤左侧字符串
     * @param string $mainStr 要过滤的主字符串
     * @param string $lfilter 主字符串左侧要过滤的子字符串
     * @return string 成功返回主字符串的右侧剩余部分，失败返回false，由于可能返回空字符串，所以请用===来检测false
     */
    public static function filterOfLeft($mainStr, $lfilter) {
        $lfilterLen = strlen($lfilter);
        if ($lfilterLen == 0)
            return $mainStr;
        if (substr($mainStr, 0, $lfilterLen) !== $lfilter)
            return FALSE;
        if (strlen($mainStr) == $lfilterLen)
            return '';
        return substr($mainStr, $lfilterLen);
    }
    /**
     * @static
     * 过滤右侧字符串
     * @param string $mainStr 要过滤的主字符串
     * @param string $rfilter 主字符串右侧要过滤的子字符串
     * @return string 成功返回主字符串的左侧剩余部分，失败返回false，由于可能返回空字符串，所以请用===来检测false
     */
    public static function filterOfRight($mainStr, $rfilter) {
        $rfilterLen = strlen($rfilter);
        if ($rfilterLen == 0)
            return $mainStr;
        if (substr($mainStr, 0 - $rfilterLen) !== $rfilter)
            return FALSE;
        return substr($mainStr, 0, 0 - $rfilterLen);
    }
    /**
     * @static
     ...
     * @return string
     */
    public static function substr($str, $start, $length,$charset='utf-8',$suffix=false,$suffix_str='…') {
        if (empty ( $str )) {
            return $str;
        }
        if(function_exists('mb_substr'))    {
            if(mb_strlen($str, $charset) <= $start+$length){
                ...
                self::fixSubstrByLength($suffix,$length,$suffix_len);
            }
    ...
    static private function fixSubstrByLength(&$suffix,&$length,$suffix_len){
     ...
     * @return string
     */
    public static function getSubstrIgnoreHtml($body, $size) {
     ...
     * @return string
     */
    public static function cn($data, $type = 0, $data_is_utf8 =true) {
