 ...
 * @version    $Id: \Library\Download.php 版本号 2013-1-1 $
 *
 */
class Download
{
    ...
    public static function getFile($file_path, $rename = NULL){
        if(DIRECTORY_SEPARATOR=='\\')$file_path=iconv("UTF-8","GB2312//IGNORE",$file_path);
        if(!file_exists($file_path)) return notice(FALSE, '你要下载的文件不存在！');
        $file_size_string=sprintf("%u", filesize($file_path));
        $file_size = intval($file_size_string);
        if(headers_sent())return notice(FALSE, '文件头已经输出了！');
    ...
    private static function _readFileOfLarge($file_path,$chunk_size=1024){
        ...
        if ($handle === false) return notice(FALSE, '不能打开这个大文件！');
        while (!feof($handle)) {
        ...
        if(!fclose($handle)) error_exit('下载文件后出错了！向浏览器发送完大文件后关闭文件失败。');
